  ...
  chrome.storage.sync.get('alarms',(items)=>{
    ...
    alarmsKey.forEach((i)=>{
      if(!isNaN(alarmsObj[i].period)){
        const template = `<p class="alarm-item"><i class="fa-solid fa-alarm-clock"></i> <b>[${i}] </b><i class="fa-light fa-right-long"></i> ${alarmsObj[i].time} <br> <b class="period">Period: ${alarmsObj[i].period} min</b> <button class="remove">remove</button></p>`;
        alarmTime.innerHTML += template;
      }else{
        const template = `<p class="alarm-item"><i class="fa-solid fa-alarm-clock"></i> <b>[${i}] </b><i class="fa-light fa-right-long"></i> ${alarmsObj[i].time} <br> <b class="period">Period: Once</b> <button class="remove">remove</button></p>`;
        alarmTime.innerHTML += template;
      }
    });
  });
        ...
        if(alarmKeys.findIndex((k)=>{return k == alarmName;}) == -1 && timeArray.findIndex((t)=>{return t == localeString;}) == -1){
          ...
          chrome.runtime.sendMessage({
           ...
           // console.log(response.status);
            alarmForm.reset();
            if(!isNaN(period)){
              const template = `<p class="alarm-item"><i class="fa-solid fa-alarm-clock"></i> <b>[${alarmName}] </b><i class="fa-light fa-right-long"></i> ${localeString} <br> <b class="period">Period: ${period} min</b> <button class="remove">remove</button></p>`;
              alarmTime.innerHTML += template;
            }else{
              const template = `<p class="alarm-item"><i class="fa-solid fa-alarm-clock"></i> <b>[${alarmName}] </b><i class="fa-light fa-right-long"></i> ${localeString} <br> <b class="period">Period: Once</b> <button class="remove">remove</button></p>`;
              alarmTime.innerHTML += template;
            }
          });
          // console.log('storage:');
